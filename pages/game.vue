...
import type { World, Agent, Entity } from "@/stores/types";
...
const visibleEntities = ref<Entity[]>([] as Entity[]);
...
onMounted(() => {
  ...
  visibleEntities.value = worldStore.worldOpts.entities;
});
    ...
    <div class="flex justify-between w-full">
      <Drawer label="World">
        ...
        >
          ...
          >
            <h2 class="relative z-10">Entities in Scene</h2>
          </button>

          <div v-if="expandEntityList" class="mt-2 space-y-3">
            <div
              v-for="entity in worldOpts!.entities"
              :key="entity.name"
      ...
      <Drawer label="Stats" right>
        <div v-if="worldOpts?.stats !== undefined" class="overflow-y-auto">
          <StatRender v-for="stat in worldOpts?.stats" :stat="stat" />
        </div>
      </Drawer>
    </div>

    <StyledTextArea class="flex-1" />
